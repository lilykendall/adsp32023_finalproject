...
trained in `fastercnn/` and `yolo/02d_colab_final_evaluation.ipynb`, and one retriever out of the
...
**Deployed checkpoint is now `highres_yolov8s_960`**, a 960px retrain of the
baseline below (see `yolo/02e_colab_highres_retrain.ipynb`). Warm-started from
`baseline_yolov8s/weights/best.pt` rather than COCO weights, so the run spent
its budget adapting to 960px instead of relearning the 148-class taxonomy.
Training stopped early at epoch 38/60 (patience 15), 53 epochs run in 4.05h,
under the 5.5h Colab budget cap.

| Protocol | test mAP@0.5 | test mAP@[0.5:0.95] | P | R |
|---|---:|---:|---:|---:|
| conf=0.25 (baseline-comparable) | **0.2506** | **0.1498** | 0.397 | 0.285 |
| conf=0.001 (COCO-style floor) | 0.3870 | 0.2184 | — | — |

Both baseline-comparable numbers clear the swap-in bar the notebook set for
itself (beat 0.2472 / 0.1461) — a modest but real gain, +1.4% mAP@0.5 and
+2.5% mAP@[0.5:0.95], not the outsized jump the resolution increase alone
might suggest. Inference measured at 3.2 ms/image on an A100 at test time
(vs. the baseline's reported 8.3 ms/image — different hardware and protocol,
so treat as indicative, consistent with the caveat below).

The class-tail problem this retrain targeted also improved: of the classes
with any test instances, zero-AP50 classes dropped from 46/102 (baseline) to
35/101 (highres), attributable to the `copy_paste=0.3` instance-paste
augmentation added for this run. Small-object recall — the other targeted
weakness — was not independently re-measured with the baseline's per-size
breakdown script; training at 960px (~2.25x the pixels per box vs. 640px) was
the lever pulled, but no separate small/medium/large recall table exists yet
for this checkpoint.

Everything below this point describes the `baseline_yolov8s` arm the highres
checkpoint was warm-started from and measured against; it's why YOLOv8s beat
the alternative architectures in the first place, which the retrain doesn't
change.
...
**It already has a deployment artifact.** `yolo/02d_colab_final_evaluation.ipynb` exports
...
- **Small objects are the weak point (baseline figures below; the deployed
  `highres_yolov8s_960` checkpoint targets this but wasn't re-measured
  per-size).** Recall@IoU0.5 by object size on the baseline: large 0.736,
  medium 0.635, **small 0.405** (24,104 of the 30,832 test boxes are small). A
  wide fridge shot where each item is small is the hard case; a closer photo
  of one shelf works better. Two mitigations stack: training/inferring at
  960px instead of 640px gives every box ~2.25x the pixels, and at inference
  time `detector.py` still tiles for uploads meaningfully larger than the
  model resolution (now 960px tiles, not 640px), scoring each item at
  something close to native resolution. On a 1094×730 composite of the demo
  image, tiling at 640px took the detector from 4 boxes to 116 (ground truth
  ~108) at roughly the same latency, since the tiles run as one
  batch.
...
(Jaccard scores 0.912 HitRate@1 in `recipe_embedding/Recipe_Modeling.ipynb` and 0.932 in
`recipe_embedding/qwen_contrastive_finetune_triplet_setups.ipynb`; the two notebooks build the
...
`fridgefest/taxonomy.py` closes that gap by canonicalising both sides before
...
lists in `fridgefest/data/ingredient_taxonomy.json`.
...
`recipe_embedding/Recipe_Modeling.ipynb`'s `_clean_ingredient` strips `freshly` as a prep verb
...
at the **display** layer only (`_PANTRY_STAPLES` in `fridgefest/recipes.py`), so the
